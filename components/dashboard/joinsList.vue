<template>
  <div>
    <v-card v-if="!items.length && !loading" flat class="py-8">
      <v-card-text>
        <div class="text-center">
          <span>
            No join requests to show, Please
            <a>retry</a>
          </span>
        </div>
      </v-card-text>
    ...
    <div v-outside-click="onClickOutside">
      <v-card
        v-for="(join, i) in items"
        :key="i"
        outlined
        class="my-2 join-card"
        :ripple="false"
        @click="actionBtns = i"
      >
        <div v-if="!!join.user">
          <v-row no-gutters>
            <v-col class="py-4 pl-4">
              ...
              <div class="text-truncate body-2 text--secondary">
                {{ join.user.major || 'None' }}
              </div>
            </v-col>
            <div
              v-show="actionBtns === i"
              class="join-action-buttons fill-height"
            >
              <v-btn
                tile
                depressed
                ...
                height="100%"
                width="80"
                @click="$emit('accept', join.user.email)"
              >
                ...
                <table>
                  <tbody>
                    ...
                    <tr>
                      ...
                      <td class="main--text">
                        {{ join.user.first + ' ' + join.user.last }}
                      </td>
                    </tr>
                    <tr>
                      <td class="pr-4 text--secondary">
                    ...
                    <tr>
                      ...
                      <td>
                        {{ join.user.uncheckedTicketsCount }}
                      </td>
                    </tr>
                    <tr>
                      <td class="pr-4 text--secondary">
...
/* .join-card:hover .join-action-buttons {
  display: inline-flex;
} */
.join-action-buttons {
  position: absolute;
